...
use oviec::{Compiler, Backend, OvieResult, OvieError, AstNode, Statement, Expression, PackageRegistry, PackageLock, DependencyResolver, ProjectConfig, SelfHostingManager, SelfHostingStage, RealBootstrapConfig, RealBootstrapResult, BrandingConfig, ProjectTemplate, ProjectMetadata, IntegrityManifest, CrossTargetValidator, CrossTargetValidationConfig};
use std::fs;
use std::path::Path;
use std::process;
...
enum SelfHostingAction {
    ...
    /// Bootstrap verification - compare Rust and Ovie lexers
    Bootstrap {
        #[command(subcommand)]
...
enum BootstrapAction {
    /// Verify that Ovie lexer produces identical output to Rust lexer
    Verify {
        /// Source file to verify (optional, uses test suite if not specified)
        file: Option<String>,
        /// Enable verbose output
        #[arg(long)]
        verbose: bool,
...
fn cmd_build(file: Option<String>, backend: String, _target: Option<String>, output: Option<String>, debug: bool, deterministic: bool, _object: bool, _assembly: bool) -> OvieResult<()> {
...
fn cmd_analyze(file: Option<String>, debug: bool, _format: String, output: Option<String>) -> OvieResult<()> {
    let source_file = file.unwrap_or_else(|| "src/main.ov".to_string());
...
fn format_statement(stmt: &Statement, output: &mut String, indent_level: usize) {
    ...
    match stmt {
        ...
        Statement::Function { name, parameters, body } => {
            let params = parameters.iter().map(|p| if p.mutable { format!("mut {}", p.name) } else { p.name.clone() }).collect::<Vec<_>>().join(", ");
        ...
        Statement::FieldMutation { object, field, value } => {
            output.push_str(&format!("{}{}.{} = {};", indent, format_expression(object), field, format_expression(value)));
        }
    }
}
...
fn cmd_self_host(action: SelfHostingAction) -> OvieResult<()> {
    match action {
        ...
        SelfHostingAction::Bootstrap { bootstrap_action } => cmd_bootstrap(bootstrap_action),
...
fn cmd_self_host_verify(test_files: Vec<String>, verbose: bool) -> OvieResult<()> {
    ...
    let config = RealBootstrapConfig {
        ...
        ovie_lexer_path: std::path::PathBuf::from("std/lexer/mod.ov"),
    };
    ...
    match manager.initialize_bootstrap_verification(config) {
        Ok(()) => {
            ...
            let test_cases: Vec<(String, String)> = if test_files.is_empty() {
                vec![
                    ("test1".to_string(), r#"seeAm "Hello, World!";"#.to_string()),
                    ("test2".to_string(), r#"name = "Ovie"; mut counter = 42;"#.to_string()),
                    ("test3".to_string(), r#"fn greet(person) { seeAm "Hello, " + person + "!"; }"#.to_string()),
                ]
            } else {
                // Read test files
            ...
            let test_case_refs: Vec<(&str, &str)> = test_cases.iter()
...
/// Handle bootstrap verification commands
fn cmd_bootstrap(action: BootstrapAction) -> OvieResult<()> {
    match action {
        BootstrapAction::Verify { file, verbose, report, benchmark } => {
            cmd_bootstrap_verify(file, verbose, report, benchmark)
        }
    }
}

/// Verify that Ovie lexer produces identical output to Rust lexer
fn cmd_bootstrap_verify(
    file: Option<String>,
    verbose: bool,
    report_path: Option<String>,
    benchmark: bool,
) -> OvieResult<()> {
    use oviec::self_hosting::bootstrap_verification::{RealBootstrapVerifier, RealBootstrapConfig};
    
    println!("🔍 Bootstrap Verification - Comparing Rust and Ovie Lexers\n");
    ...
    let test_cases: Vec<(&str, &str)> = if let Some(file_path) = file {
        // Single file verification
        let source = fs::read_to_string(&file_path)
            .map_err(|e| OvieError::runtime_error(format!("Failed to read file: {}", e)))?;
        vec![(Box::leak(source.into_boxed_str()), Box::leak(file_path.into_boxed_str()))]
    } else {
        // Default test suite
        vec![
            ("seeAm \"hello\";", "simple_print"),
            ("let x = 42;", "variable_declaration"),
            ("fn main() { }", "empty_function"),
            ("let x = 42;\nlet y = 100;\nseeAm x + y;", "multi_statement"),
            ("fn add(a, b) { return a + b; }", "function_with_params"),
            ("if true { seeAm \"yes\"; } else { seeAm \"no\"; }", "if_else"),
            ("for i in 0..10 { seeAm i; }", "for_loop"),
            ("// Comment\nseeAm \"test\";", "with_comment"),
            ("struct Point { x, y }", "struct_definition"),
            ("enum Color { Red, Green, Blue }", "enum_definition"),
        ]
    };
    
    println!("🧪 Running verification on {} test case(s)...\n", test_cases.len());
    ...
    if benchmark {
        let perf_ratios: Vec<f64> = results.iter()
            .filter(|r| r.performance_ratio > 0.0)
            .map(|r| r.performance_ratio)
            .collect();
        
        if !perf_ratios.is_empty() {
            let avg_ratio = perf_ratios.iter().sum::<f64>() / perf_ratios.len() as f64;
            println!("\n⚡ Performance:");
            println!("  Average ratio: {:.2}x (Ovie/Rust)", avg_ratio);
        }
    }
    ...
    if let Some(report_file) = report_path {
        ...
        fs::write(&report_file, report)
            .map_err(|e| OvieError::runtime_error(format!("Failed to write report: {}", e)))?;
        println!("✅ Report saved to: {}", report_file);
    }
    
    // Final status
    if passed == total {
        println!("\n🎉 REAL SELF-HOSTING ACHIEVED!");
        println!("   The Ovie lexer written in Ovie produces identical output to the Rust lexer.");
    } else {
        println!("\n⚠️  Self-hosting not yet complete.");
        println!("   Further development needed to achieve full bootstrap verification.");
    }
